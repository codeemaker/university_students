    ...
    try{
        ...
        if(name && course && batch && universityId && email && password){
            ...
            if(findName){
                ...
                let updatedObj = {...req.body,password:hashedpassword,status:true,profile:req.file ? req.file.path : ""};
...
const getAllStudents = async(req,res)=>{
    try{
        let univData = await StudentModel.find({status:true},{password:0});
...
const updateStudents = async(req,res)=>{
    try{
        ...
        await StudentModel.updateOne({_id:id},updatedObj);
...
const deleteStudents = async (req, res) => {
    try {
        ...
        const result = await StudentModel.updateOne({ _id: id },{ status:false });
...
export { createStudent, getAllStudents, updateStudents, deleteStudents };